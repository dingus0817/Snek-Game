...
snek1 = pygame.transform.scale(snek1,(380,380))
snek2 = pygame.image.load("../SnekImages/snek_snek2.png")
snek2 = pygame.transform.scale(snek2,(380,380))
...
def drawMenu():
    ...
    introText1 = smallFont.render("use the arrows keys to move",1,WHITE)
    introText2 = smallFont.render("eat as many apples as you can",1,WHITE)
    playText = meFont.render("press SPACE to play",1,LIGHT_BLUE)
    optionsText = meFont.render("press S for options",1,LIGHT_BLUE)
    quitText = meFont.render("press ESC to quit",1,LIGHT_BLUE)
    gameWindow.blit(titleText,(100,70))
    gameWindow.blit(introText1,(100,180))
    gameWindow.blit(introText2,(100,210))
    gameWindow.blit(playText,(50,330))
    gameWindow.blit(optionsText,(50,440))
    gameWindow.blit(quitText,(50,550))
...
def drawMenuSnek():
    ...
    menuSnek1 = False
    if menuSnek1:
        gameWindow.blit(snek1,(410,290))
    else:
        gameWindow.blit(snek2,(410,290))

def initGame():
    global headCLR,segCLR,SEGMENT_R,HSTEP,VSTEP,stepX,stepY,segX,segY,tailX,tailY,eyes,eyeX,eyeY,applesX,applesY,nom,eatenApples
    global obstaclesX,obstaclesY,score,count,totalSec,sec,clock,BEGIN_CLOCK,BEGIN_APPLE,PERIOD,referenceTime,FPS,begin_time
    # snake's properties
    headCLR = GREEN
    segCLR = DARK_GREEN
    SEGMENT_R = 10
...
while inPlay:
    while inMenu:
        pygame.event.pump()
        elapsedSnek = round(time.time() - referenceTime,1)
        if elapsedSnek > snekPeriod:
            menuSnek1 = True
        drawMenu()
        drawMenuSnek()
